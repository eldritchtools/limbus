...
export default function ReviewsComponent({ type, id, sortType, userReview, guardedLinks }) {
    ...
    return <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", minWidth: "min(480px, 100%)", flex: 1 }}>
        {userReview?.review_text && <>
            ...
            <Review
                type={type}
                reviewData={userReview} backReview={userReview}
                usernameOverride={profile?.username}
                userAvatarIdOverride={profile?.avatar_id}
        ...
        }
        {
            reviews
                ...
                .map(review => <Review key={review.id} type={type} reviewData={review} backReview={review} guardedLinks={guardedLinks} />)
        }
